...
function BarHour($hour){
    ...
    <div class="hour" hour="<?=$hour['hour'];?>">
        <div var="temp" style="top:<?=map(-10,120,100,0,$hour['temp_max']+1);?>%; bottom:<?=map(120,-10,100,0,$hour['temp_min']-1)?>%; background-color:<?=interpolateColor(Colors::GetColor("temp_".floor($hour['temp']/10)),Colors::GetColor("temp_".ceil($hour['temp']/10)),($hour['temp']/10)-floor($hour['temp']/10));?>" title="Indoors -- <?=Times24ToTime12Short($hour['hour'])?>\nTemp: <?=round($hour['temp']);?>° | <?=round($hour['temp_max']);?>° / <?=round($hour['temp_min']);?>°\nHum: <?=round($hour['hum']);?>% | <?=round($hour['hum_max'])?>% / <?=round($hour['hum_min']);?>%"></div>
    </div>
    <?php
}
?>
...
<div class="temp_chart indoors bar" collection="indoor_temperature" room_id="<?=$_GET['room_id'];?>">
    <div class="time_bar"></div>
